...
class Level:
    def __init__(self, window: Surface, name: str, game_mode: str, player_score: list[int]):
        # Define a duração do nível (Level 3 tem o dobro da duração dos outros)
        ...
        while True:
            ...
            for event in pygame.event.get():
                ...
                if event.type == EVENT_ENEMY:
                    # No Level 3, apenas `Enemy3` é gerado
                    if self.name == 'Level3':
                        self.entity_list.append(EntityFactory.get_entity('Enemy3'))
                    else:
                        # Nos outros níveis, alterna entre Enemy1 e Enemy2
                ...
                # Verifica se algum jogador ainda está vivo
                found_player = any(isinstance(ent, Player) for ent in self.entity_list)
                if not found_player:
                    return False


            # Informações do nível e da tela
            ...
            # Verificação de colisões e saúde
